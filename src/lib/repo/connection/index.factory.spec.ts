...
>(anyTest: TestInterface, connectionRepositoryFactory: () => Promise<CR>) {
  ...
  test('Should be able to get connections of group', async (t) => {
    ...
    const connectionsIds = [
      { id: 'a', hasPrivilege: false },
      { id: 'b', hasPrivilege: true },
    ];

    await t.context.repo.add(
      groupId,
      connectionsIds[0].id,
      connectionsIds[0].hasPrivilege
    );
    await t.context.repo.add(
      groupId,
      connectionsIds[1].id,
      connectionsIds[1].hasPrivilege
    );
    await t.context.repo.add('_', '_', false);

    t.deepEqual(
      await t.context.repo.getConnectionsOfGroup(groupId),
      connectionsIds
    );
  });
  ...
  test('Should be able to check whether a connection has editing privilege', async (t) => {
    ...
    t.is(
      await t.context.repo.hasPrivilege(connectionsIds[0].id),
      connectionsIds[0].hasPrivilege
    );
    t.is(
      await t.context.repo.hasPrivilege(connectionsIds[1].id),
      connectionsIds[1].hasPrivilege
    );
